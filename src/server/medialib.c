...
static struct channel_context_st* path2entry(const char *path){
	...
	if(glob(pathstr , 0 , NULL , &me->mp3glob) != 0){
		curr_id++;
		syslog(LOG_ERR , "%s is not a channel dir(can`t fine mp3 file)",path);
		free(me);
		return NULL;
	}
...
static int open_next(chnid_t chnid){
	...
	syslog(LOG_ERR , "None of mp3 in channel %d  is avliveable",channel[chnid].chnid);
}

ssize_t mlib_readchn(chnid_t chnid ,void* buf ,size_t size){
	
	int tbfsize , len;
	tbfsize = mytbf_ferchtoken(channel[chnid].tbf , size);
	while(1){
		len = pread(channel[chnid].fd , buf ,tbfsize , channel[chnid].offset);
		if(len < 0){
			syslog(LOG_WARNING , "media file %s pread faile: %s" , channel[chnid].mp3glob.gl_pathv[channel[chnid].pos],strerror(errno));
			open_next(chnid) ;
		}
		else if(len == 0){
